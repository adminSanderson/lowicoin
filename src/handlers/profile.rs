...
pub async fn handle_profile(api: &Api, message: Message) {
    ...
    let balance = crate::db::get_user_balance(user_id as i64).unwrap_or(0.0);
    let idpay = crate::db::get_user_idpay(user_id as i64).unwrap_or(None).unwrap_or_else(|| "Unknown".to_string());
