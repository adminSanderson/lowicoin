...
pub async fn handle_pay(api: &Api, message: Message) {
    ...
    if parts.len() < 3 {
        let params = SendMessageParams {
            ...
            text: "❌ Incorrect command format. Example: /pay 2.5 (coins) jnRHDtjlCfhNfc5b (Pay id)".to_string(),
            parse_mode: None,
            entities: None,
    ...
    let coin_amount = match parts[1].parse::<f64>() {
        Ok(val) => val,
        Err(_) => {
            let params = SendMessageParams {
                chat_id: frankenstein::ChatId::Integer(message.chat.id),
    ...
    if coin_amount < 0.01 {
        let params = SendMessageParams {
            chat_id: frankenstein::ChatId::Integer(message.chat.id),
            text: "❌ Minimum transfer amount: 0.01 coin.".to_string(),
            parse_mode: None,
            entities: None,
            disable_web_page_preview: None,
            disable_notification: None,
    ...
    let sender_balance = crate::db::get_user_balance(sender_id as i64).unwrap_or(0.0);
    ...
    let (success, new_sender_balance) = crate::db::transfer_coins(sender_id as i64, user_key, coin_amount).unwrap_or((false, None));
    if !success {
        let params = SendMessageParams {
            chat_id: frankenstein::ChatId::Integer(message.chat.id),
            text,
