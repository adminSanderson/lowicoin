...
pub fn add_user(user_id: i64, id_pay: &str, coins: f64) -> Result<()> {
    let conn = connect()?;
    conn.execute(
...
pub fn get_user_balance(user_id: i64) -> Result<f64> {
    let conn = connect()?;
...
pub fn get_user_idpay(user_id: i64) -> Result<Option<String>> {
    let conn = connect()?;
    let idpay: Option<String> = conn.query_row(
...
pub fn transfer_coins(sender_id: i64, receiver_idpay: &str, coin_amount: f64) -> Result<(bool, Option<f64>)> {
    let mut conn = connect()?;
    let tx = conn.transaction()?;

    let sender_balance: Option<f64> = tx.query_row(
        "SELECT coins FROM Users_coins WHERE id_users = ?1",
        [sender_id],
        |row| row.get(0)
    ).optional()?;
    let sender_balance = sender_balance.unwrap_or(0.0);
    if sender_balance < coin_amount {
        return Ok((false, None));
    }

    let receiver_balance: Option<f64> = tx.query_row(
